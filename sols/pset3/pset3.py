...
def getAvailableLetters(lettersGuessed):
    ...
    """
    return ''.join(filter(lambda x: x not in lettersGuessed, string.ascii_lowercase))
...
def hangman(secretWord):
    ...
    * Accept only single alphabetic characters; ignore/reprompt otherwise.
    ...
    while guesses_left > 0:
        ...
        print(f"Letters available: {getAvailableLetters(letters_guessed)}")
        print(f"The gallows:\n{getGuessedWord(secretWord, letters_guessed)}")
        letter = input("Insert guess: ")
        if len(letter) != 1:
            print(f"This is not a letter! {letter}")
            continue
        elif letter.lower() not in getAvailableLetters(letters_guessed):
            print(f"Letter {letter.lower()} is not available")
            continue
